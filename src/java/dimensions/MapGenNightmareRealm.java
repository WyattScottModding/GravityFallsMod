...
public class MapGenNightmareRealm extends MapGenStructure
{
    private final int citySpacing = 20;
    private final int minCitySeparation = 11;
    private final ChunkGeneratorNightmareTest endProvider;

    public MapGenNightmareRealm(ChunkGeneratorNightmareTest p_i46665_1_)
    {
    ...
    {
        return new MapGenNightmareRealm.Start(this.world, this.endProvider, this.rand, chunkX, chunkZ);
    }
    ...
    private static int getYPosForStructure(int p_191070_0_, int p_191070_1_, ChunkGeneratorNightmareTest p_191070_2_)
    {
    ...
    public static class Start extends StructureStart
        {
            ...
            public Start(World worldIn, ChunkGeneratorNightmareTest chunkProvider, Random random, int chunkX, int chunkZ)
            {
                super(chunkX, chunkZ);
                this.create(worldIn, chunkProvider, random, chunkX, chunkZ);
            }

            private void create(World worldIn, ChunkGeneratorNightmareTest chunkProvider, Random rnd, int chunkX, int chunkZ)
            {
                Random random = new Random((long)(chunkX + chunkZ * 10387313));
                Rotation rotation = Rotation.values()[random.nextInt(Rotation.values().length)];
                int i = MapGenNightmareRealm.getYPosForStructure(chunkX, chunkZ, chunkProvider);
