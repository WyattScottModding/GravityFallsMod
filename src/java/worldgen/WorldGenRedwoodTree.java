...
{
	public static final Block LOG = BlockInit.REDWOOD_LOGS;
	public static final Block LEAF = BlockInit.REDWOOD_LEAVES;
	...
	int heightLimit;
	int height;
	double heightAttenuation = 0.8D;
	double branchSlope = 0.1D;
	double scaleWidth = 0.6D;
	double leafDensity = 0.7D;
	int trunkSize = 1;
	int heightLimitLimit = 12;
	/** Sets the distance limit for how far away the generator will populate leaves from the base leaf node. */
	int leafDistanceLimit = 6;
	List<WorldGenRedwoodTree.FoliageCoordinates> foliageCoords;
	...
	void generateLeafNodeList()
	{
		...
		this.foliageCoords = Lists.<WorldGenRedwoodTree.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenRedwoodTree.FoliageCoordinates(this.basePos.up(k), j));
			...
			if (f >= 0.0F)
			{
				for (int l = 0; l < i; ++l)
				{
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1)
					{
						...
						{
							this.foliageCoords.add(new WorldGenRedwoodTree.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void generateLeafNode(BlockPos pos)
	{
		...
		{
			this.crosSection(pos.up(i), this.leafSize(i), LEAF.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
		}
	}

	boolean limb(BlockPos pos1, BlockPos pos2, Block block, boolean branch)
	{
		Block blockbase = world.getBlockState(pos1.down()).getBlock();
		if(!branch && blockbase != Blocks.GRASS)
		{
			for(int k = 0; k <= pos1.getY(); k++)
			{
				//Fixes tree bug where trees spawn too high up
				if(blockbase != Blocks.GRASS)
				{
					pos1 = pos1.down();
					pos2 = pos2.down();
					blockbase = world.getBlockState(pos1.down()).getBlock();
				}
				else
					break;
			}

			//If there is still an issue, return false and cancel the tree
			if(blockbase != Blocks.GRASS)
			{
				return false;
			}
	...
	void generateLeaves()
	{
		for (WorldGenRedwoodTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
		{
	 ...
	 */
	boolean generateTrunk()
	{
		BlockPos blockpos = this.basePos;
		BlockPos blockpos1 = this.basePos.up(this.height);

		boolean treeValid = this.limb(blockpos, blockpos1, LOG, false);

		if(treeValid)
		{
			if (this.trunkSize >= 2)
			{
				this.limb(blockpos.east(), blockpos1.east(), LOG, true);
				this.limb(blockpos.east().south(), blockpos1.east().south(), LOG, true);
				this.limb(blockpos.south(), blockpos1.south(), LOG, true);
			}
			if (this.trunkSize >= 3)
			{
				this.limb(blockpos.east().east(), blockpos1.east().east(), LOG, true);
				this.limb(blockpos.east().east().south(), blockpos1.east().east().south(), LOG, true);
				this.limb(blockpos.east().east().south().south(), blockpos1.east().east().south().south(), LOG, true);
				this.limb(blockpos.east().south().south(), blockpos1.east().south().south(), LOG, true);
				this.limb(blockpos.south().south(), blockpos1.south().south(), LOG, true);
			}
		}
		return treeValid;
	}
	...
	void generateLeafNodeBases()
	{
		for (WorldGenRedwoodTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
		{
			...
			{
				this.limb(blockpos, worldgenbigtree$foliagecoordinates, LOG, true);
			}
		}
	}
	...
	public boolean generate(World worldIn, Random rand, BlockPos position)
	{
		...
		this.trunkSize = 1 + rand.nextInt(3);

		if(this.trunkSize == 1)
			this.heightLimit = 12 + rand.nextInt(5);
		else if(this.trunkSize == 2)
			this.heightLimit = 16 + rand.nextInt(9);
		else if(this.trunkSize == 3)
			this.heightLimit = 24 + rand.nextInt(13);
		...
		else
		{
			...
			boolean generated = this.generateTrunk();
			if(generated)
			{
				this.generateLeaves();
	...
	/**
	 * Returns a boolean indicating whether or not the current location for the tree, spanning basePos to the height
	 * limit, is valid.
	 */
